...
#check missing values SEE <chr> variables
colSums(is.na(sparcs)) #hospital_id=0, age_group = 0, sex=0, race=0, discharge_status = 0, payer_primary = 64121 ed_indicator=64015

#count missing values in <int> variables
sum(is.na(sparcs$discharge_year)) #=0
sum(is.na(sparcs$apr_drg_code)) #=64015
sum(is.na(sparcs$apr_mdc_code)) #=64015
sum(is.na(sparcs$apr_severity_code)) #=64015
...
#change class variables to factor (here only factor variables) 
#variables of interest = sex, race, discharge_status, ccsr_proc, payer_primary and ed_indicator
sparcs_hf <- sparcs_hf |>
  dplyr::mutate(
...
#create variable ed_indicator Yes and No  
sparcs_hf <- sparcs_hf |>
  mutate(ed_admission = ifelse(
    ed_indicator == "Y",  "Emergency",
                          "Non-emergency"))
#count new levels ed_indicator
table(sparcs_hf$ed_indicator)
#convert ed_indicator as factor
sparcs_hf$ed_indicator <- factor(sparcs_hf$ed_indicator)
#count ed_indicator N=2076 Y=25861
table(sparcs_hf$ed_indicator)

#change class variables to numeric (here only numeric) = los, costs and charges
sparcs_hf <- sparcs_hf |>
  dplyr::mutate(
    los = as.numeric(los),
...
#remove empty levels (variables as factor)
sparcs_hf <- droplevels(sparcs_hf)
#check impossible values min=1 max=118
min(sparcs_hf$los)
max(sparcs_hf$los)

#check number of hospitals = 178
...
#summary hospital n of admissions min 1 max 512 median128 mean 156
sparcs_hf |>
  count(hospital_id) |>
  summary()
...
#emergency = 25852 and non-emergency = 2073 
table(sparcs_hf$ed_admission)

#check again missing values in numeric categories of interest= los/charges/costs
sum(is.na(sparcs_hf$los))
sum(is.na(sparcs_hf$charges))
sum(is.na(sparcs_hf$costs))

#check again NO missing values in factor categories of interest= hospital_id, age_group, sex, apr_drg,
#apr_severity, race_group, discharge_group, proc_group, payer_group and ed_admission
